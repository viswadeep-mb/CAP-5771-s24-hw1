...
function_names = [f'question4_{i}' for i in range(1, 6)]
...
def test_question4_list_return(func_name, subquestions):
...
@pytest.mark.parametrize("func_name", function_names)
def test_question4_word_in_list(func_name, subquestions):
    # Use getattr to dynamically get the function from the module q
    func = getattr(q, func_name)
    answer = func()
    ...
    for word in answer:
        assert word.lower().strip() in accepted_words, f"{word} is not in {accepted_words}"
